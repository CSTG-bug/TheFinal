...
================================================================
功能：
  ✅ 读取预处理后的四个 CSV（X_train, y_train, X_test, y_test）
  ✅ Pipeline：StandardScaler + KNeighborsRegressor（避免数据泄漏）
  ✅ 网格搜索（5 折交叉验证），评分：R²
...
def train_eval_and_maybe_save(
    ...
    # ---------- 2) 参数网格 ----------
    # 说明：
    #  - n_neighbors：K 的大小；一般奇数更稳（避免分类里平票，回归无硬性要求）
    #  - weights：'uniform'（等权） vs 'distance'（按距离加权）
    #  - p：Minkowski距离的幂；p=1 曼哈顿，p=2 欧氏
    param_grid = {
        "knn__n_neighbors": [3, 5, 7, 9, 11, 15],
        "knn__weights": ["uniform", "distance"],
        "knn__p": [1, 2],
        # 如需更快，可减少候选；如需更强，可加入 leaf_size、metric 等
    }
    ...
    # 9.1 metrics.json
    metrics_path = outdir / "KNN_metrics.json"
    ...
    # 9.4 最佳模型
    if save_model:
        dump(best_model, outdir / "KNN_best_model.joblib")

    print(f"\n✅ 已保存到：{outdir.resolve()}")
    print("- KNN_metrics.json\n- KNN_predictions.csv")
    if save_cv_results: print("- KNN_cv_results.csv")
    if save_model: print("- KNN_best_model.joblib")
