...
def render_advanced_page(feature_names_uts, process_cols):
    ...
    # --- EL ---
    with tabs[0]:
        ss.use_el = st.checkbox("计算并展示延伸率", value=ss.use_el)
        if ss.use_el:
            ss.constrain_el = st.checkbox("添加约束：延伸率 ≥ 目标值", value=ss.constrain_el)
            if ss.constrain_el:
                ss.target_el = st.number_input("目标 延伸率（%）", min_value=0.0, value=float(ss.target_el), step=0.5)
    ...
    with tabs[4]:
        st.caption("留空表示使用默认范围")
        proc_rows = [{"工艺": f, "最小值": None, "最大值": None} for f in process_cols]
    ...
    with tabs[5]:
        st.caption("设置候选保留数量。")
        ss.archive_max_rows = st.number_input("候选保留数量", min_value=1, value=int(ss.archive_max_rows), step=200)
...
if start:
    ...
    if mode == "max":
        ...
        status.write(f"极限模式：将按 UTS_pred 从高到低导出前 {len(export_df)} 条候选。")
    else:
        ...
        if len(df_in) == 0:
            ...
            status.write("提示：区间内没有候选方案，将展示并导出“最接近该区间”的候选集合。")
        else:
            ...
            status.write(f"区间模式：已找到区间内候选 {len(df_in)} 条，将按 UTS_pred 从高到低导出前 {len(export_df)} 条。")
