...
def _feasibility_check_sum100(
    ...
    """检查成分上下限是否存在可行解（考虑 Al 在[min,max]）"""
...
def _repair_sum100_strict(
    ...
    """
    强约束：元素总和=100，同时保证 Al 在 [min,max]。
    若 Al.max < 100，需要确保 other_sum >= 100 - Al.max；这里会把“缺口”分配给仍有余量的元素。
    """
    ...
    # 若 other_sum 太小 -> 把缺口分配给还有 headroom 的元素
    ...
    # 最后再把 other 按余量回补一次（保证严格=100）
    # 若 Al 因上下限裁剪导致偏差，把偏差均摊到仍有余量的 other 元素上
    sum_other = float(np.sum(x[other_js]))
    total = sum_other + float(x[al_j])
    err = target_sum - total
    if abs(err) > 1e-8 and other_js:
        # 往上补 err：找有 headroom 的；往下减 err：找可减少的
        if err > 0:
...
def run_ga_elite_archive(
    *,
    ...
    def compute_fitness(pop_arr: np.ndarray, uts_arr: np.ndarray) -> np.ndarray:
        """返回 GA 的适应度（越大越好）。支持两种模式：
        - objective='max'：最大化 UTS_pred
        - objective='range_max'：让 UTS_pred 落在 [uts_low, uts_high] 区间内，并在区间内尽量大（输出更接近上限）
        - objective='range_mid'：让 UTS_pred 落在 [uts_low, uts_high] 区间内，并尽量靠近区间中值（更均匀）
        """
        ...
        # 同分择优：Ageing Time 越短越好（很小的 eps 作为 tie-break）
...
# 初始化
    ...
    for gen in range(1, int(max_generations) + 1):
        ...
        best = float(uts_pred[0])  # 按 objective 排序后的“当前最优方案”的 UTS_pred
        mean = float(uts_pred.mean())
        best_obj = float(fitness[0])  # objective 对应的最优值（用于提前停止判据）
