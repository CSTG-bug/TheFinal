...
def train_eval_and_maybe_save(
    ...
    # ---------- 1) 定义 Pipeline ----------
    pipe = Pipeline([
        ("scaler", FunctionTransformer(lambda X: X, validate=False) if USE_IDENTITY_SCALER else StandardScaler()),
        ("svr", SVR())
    ])
    ...
    param_grid = [
        {  # RBF
            "svr__kernel": ["rbf"],
            "svr__C": [0.3, 1, 3, 10, 30, 100, 300],
            "svr__epsilon": [0.005, 0.01, 0.03, 0.1, 0.3],
            "svr__gamma": ["scale", 0.03, 0.01, 0.003, 0.001],
        },
        {  # Linear
            "svr__kernel": ["linear"],
            "svr__C": [0.3, 1, 3, 10, 30, 100, 300],
